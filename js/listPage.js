...
define(["util","layer","editPage","qnPage","resultPage"],function(util,layer,editPage,qnPage,resultPage){
	...
	List.prototype = {
		...
		editBtnEvent : function(num){
			if(this.qnState[num]=="未发布"){
				...
				var edit= new editPage.Edit({//未点击保存时,localStorage中的内容不变
					num : num,
					storage : content,  
					head : title 
				});
			    edit.refresh();
				//Edit()有参数传递时,根据参数渲染页面,否则渲染无问题的初始页面
			}else{
				layer.layerOut({
					head : "提示",
